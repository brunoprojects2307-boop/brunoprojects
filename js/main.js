...
function renderProductsByCategory() {
  ...
  // Sort keys so 'Projects' is first
  const categoryKeys = Object.keys(productsByCategory).sort((a, b) => {
...
function createProductCard(product) {
  ...
  return `
    <div class="product-card fade-in ${isOutOfStock ? 'out-of-stock' : ''}" onclick="goToProduct(${product.id})">
      ...
      <div class="product-info">
        ...
        <div class="product-actions">
          <button class="btn btn-outline" onclick="event.stopPropagation(); addToCart(${product.id})" ${isOutOfStock ? 'disabled style="opacity: 0.5; cursor: not-allowed;"' : ''}>
...
function addToCart(productId) {
  ...
  if (product) {
    if (product.stock <= 0) {
      cartManager.showNotification('❌ This product is out of stock');
      return;
    }
